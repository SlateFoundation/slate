...
{foreach item=Report from=$data}
	...
	<article class="narrative">
		...
		<!-- begin standards -->
        {$standardsGrades = $Report->getStandardsGrades()}
		{if $Report->getWorksheetAssignment() && count($standardsGrades)}
						...
						<tbody>
							{foreach item=Grade from=$standardsGrades}
								<tr>
									...
									<td class="grade">{if $Grade->Grade != "N/A"}{$gradeLabels[$Grade->Grade]}{else}{$Grade->Grade}{/if}</td>
								</tr>
							{/foreach}
						</tbody>
					</table>
