...
//              dropArea.addCls('dropping');
    ...
    onPreviewItemClick: function(view, record) {
        ...
        if (record && record.get('file') && !record.get('error')) {
            url = URL.createObjectURL(record.get('file'));
            Jarvus.ux.LightBox.open({
                href: url,
        ...
        var me = this,
            defaultItems = [{
                ...
                items: [{
                    ...
                    autoScroll: true,
                    store: {
                        fields: ['filename', 'previewUrl', 'error', 'loading', 'file'],
                        data: []
                    },
                        ...
                        '<tpl for=".">',
                            '<li class="upload-preview <tpl if="loading">loading</tpl>" style="background-image:url({previewUrl})">',
                                '<tpl if="!error && !loading && file">',
                                    '<a href="javascript:void(0)" title="{filename}" class="preview-button fa fa-eye fa-2x" title="Preview {filename}"></a>',
                                '</tpl>',
