/**
 * @abstract
 * Shared implementation for the progress report authoring sections — the
 * interims and terms controllers are identical apart from their module
 * routes, stores, and view xtypes, declared via the contract below.
 *
 * ## Responsibilities
 * - Enable creating / editing section reports for students
 * - Enable creating / editing section notes
 */
Ext.define('SlateAdmin.controller.progress.AbstractReportController', {
    extend: 'Ext.app.Controller',


    // subclass contract
    moduleRoute: null, // e.g. 'progress/interims/report'
    editorFormXtype: null, // e.g. 'progress-interims-editorform'
    moduleStoreIdPrefix: null, // e.g. 'progress.interims'
    ...
    init: function() {
        ...
        // multi-match selector can't be a ref; bind against the module's
        // editor form xtype at runtime instead
        comboControl[me.editorFormXtype + ' combo'] = {
            select: 'onComboSelect'
        };
        me.control(comboControl);
    },


    // controller methods
    getModuleStore: function(name) {
        return Ext.getStore(this.moduleStoreIdPrefix + '.' + name);
    },
    ...
    onSectionsStoreLoad: function() {
        var me = this,
            ...
            reportsStore = me.getModuleStore('Reports'),
            editorForm = me.getEditorForm();
    ...
    onStudentSelect: function (studentsGrid, student) {
        ...
        if (!report) {
            ...
            report = me.getModuleStore('Reports').add({
    ...
    onSectionNotesSaveBtnClick: function() {
        var sectionNotesForm = this.getSectionNotesForm(),
            ...
            section = this.getModuleStore('Sections').getById(sectionDataRecord.get('SectionID'));
    ...
    syncSections: function() {
        var me = this,
            sectionsStore = me.getModuleStore('Sections'),
    ...
    syncReportsToStudents: function() {
        var me = this,
            section = me.getSectionsGrid().getSelection()[0],
            termsStore = me.getTermsStore(),
            studentsStore = me.getModuleStore('Students'),
            reportsStore = me.getModuleStore('Reports'),
