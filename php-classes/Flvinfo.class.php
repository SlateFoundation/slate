<?php
 ...
 class Flvinfo
 {
    ...
    public function getInfo($filename, $extended=false, $useMeta=true)
    {
            ...
            do {
                ...
                switch ($tagInfo['type']) {
                    ...
                    case 0x12:    // Meta tag
                        ...
                        if (((isset($hasMeta)) && ($hasMeta == true)) || ($useMeta == false)) {
                            continue;
                        }
        ...
        if ((isset($info->meta[1])) && (isset($info->meta[1]['duration']))) {
            $ret->duration = $info->meta[1]['duration'];
        }
        ...
        if ($ret->hasVideo) {
            ...
            } elseif ((isset($info->meta)) && (isset($info->meta[1])) && (isset($info->meta[1]['width']))) {
            ...
            } elseif ((isset($info->meta)) && (isset($info->meta[1])) && (isset($info->meta[1]['height']))) {
            ...
            if ((isset($info->meta)) && (isset($info->meta[1]))) {
                ...
                if (!isset($ret->video->codec)) {
                    $ret->video->codec = $info->meta[1]['videocodecid'];
                }


                // Bitrate
                if (isset($info->meta[1]['videodatarate'])) {
                    $ret->video->bitrate = $info->meta[1]['videodatarate'];
                }
                ...
                // Cue points
                if (isset($info->meta[1]['cuePoints'])) {
                    $ret->video->cuepoints = $info->meta[1]['cuePoints'];
                }
            }
        ...
        if ($ret->hasAudio) {
            ...
            if (isset($info->meta[1])) {
                // Codec (if not found from video frame)
                if (!isset($ret->audio->codec)) {
                    $ret->audio->codec = $info->meta[1]['audiocodecid'];
                }

                // Bitrate
                if (isset($info->meta[1]['audiodatarate'])) {
                    $ret->audio->bitrate = $info->meta[1]['audiodatarate'];
                }
            }
