...
class Signature
{
  ...
  <ds:SignedInfo>
    ...
    public function __construct()
    {
        ...
        $sigdoc->loadXML(Signature::template);
    ...
    private function getXPathObj()
    {
        if (empty($this->xPathCtx) && !empty($this->sigNode)) {
            ...
            $xpath->registerNamespace('secdsig', Signature::XMLDSIGNS);
            $this->xPathCtx = $xpath;
        }
    ...
    static function generate_GUID($prefix = 'pfx')
    {
        ...
        $guid = $prefix . substr($uuid, 0, 8) . "-" . substr($uuid, 8, 4) . "-" . substr($uuid, 12, 4) . "-" . substr(
            $uuid,
            16,
            4
        ) . "-" . substr($uuid, 20, 12);
    ...
    public function createNewSignNode($name, $value = null)
    {
        ...
        if (!is_null($value)) {
            $node = $doc->createElementNS(Signature::XMLDSIGNS, $this->prefix . ':' . $name, $value);
        } else {
            $node = $doc->createElementNS(Signature::XMLDSIGNS, $this->prefix . ':' . $name);
        }
    ...
    public function calculateDigest($digestAlgorithm, $data)
    {
        switch ($digestAlgorithm) {
            ...
            case Signature::SHA256:
                $alg = 'sha256';
                break;
            case Signature::SHA384:
                $alg = 'sha384';
                break;
            case Signature::SHA512:
                $alg = 'sha512';
                break;
            case Signature::RIPEMD160:
                $alg = 'ripemd160';
                break;
            default:
    ...
    private function addRefInternal($sinfoNode, $node, $algorithm, $arTransforms = null, $options = null)
    {
        ...
        if (is_array($arTransforms)) {
            foreach ($arTransforms AS $transform) {
                ...
                if (is_array(
                    $transform
                ) && (!empty($transform['http://www.w3.org/TR/1999/REC-xpath-19991116'])) && (!empty($transform['http://www.w3.org/TR/1999/REC-xpath-19991116']['query']))
                ) {
    ...
    public function locateKey($node = null)
    {
        ...
        if ($doc = $node->ownerDocument) {
            ...
            if ($algorithm) {
                try {
                    $objKey = new Key($algorithm, array('type' => 'public'));
                } catch (\Exception $e) {
                    return null;
                }
    ...
    {
        $certs = Signature::staticGet509XCerts($cert, $isPEMFormat);
        if (!empty($certs)) {
            return $certs[0];
        }
    ...
    static function staticAdd509Cert(
        ...
        if (!$keyInfo) {
            $inserted = false;
            $keyInfo = $baseDoc->createElementNS(Signature::XMLDSIGNS, 'ds:KeyInfo');
        ...
        $x509DataNode = $baseDoc->createElementNS(Signature::XMLDSIGNS, 'ds:X509Data');
        ...
        foreach ($certs as $X509Cert) {
            if ($issuerSerial) {
                if ($certData = openssl_x509_parse(
                    ...
                    if ($issuerSerial && !empty($certData['issuer']) && !empty($certData['serialNumber'])) {
                        ...
                        $x509IssuerNode = $baseDoc->createElementNS(Signature::XMLDSIGNS, 'ds:X509IssuerSerial');
                        $x509DataNode->appendChild($x509IssuerNode);

                        $x509Node = $baseDoc->createElementNS(Signature::XMLDSIGNS, 'ds:X509IssuerName', $issuerName);
            ...
            $x509CertNode = $baseDoc->createElementNS(Signature::XMLDSIGNS, 'ds:X509Certificate', $X509Cert);
