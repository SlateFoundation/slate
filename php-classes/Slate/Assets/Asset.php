...
class Asset extends \ActivityRecord
{
    ...
    public static $searchConditions = array(
        ...
        ,'Status' => array(
            'qualifiers' => array('status', 'assets-status')
            ,'points' => 1
            ,'join' => array(
...
#        ),
    ...
    public static function sortIdentifier($dir, $name)
    {
        ...
        return array("(SELECT Identifier FROM aliases WHERE Type = '${name}' AND ObjectClass IN ('Slate\\\Assets\\\Asset', 'Asset') AND ObjectID = ${tableAlias}.ID LIMIT 1)  " . $dir);
    }
    ...
    public static function sortEntity($dir, $name)
    {
        $tableAlias = static::getTableAlias();
        return array(
            " IF(${name}ID IS NULL, 1, 0)",
            " IF(${name}Class IN ('Person', 'Emergence\\\People\\\Person', 'User', 'Emergence\\\People\\\User'), (SELECT CONCAT(LastName,' ',FirstName) FROM people Person WHERE Person.ID = ${tableAlias}.${name}ID)",
            " (SELECT Name FROM groups Grp WHERE Grp.ID = ${tableAlias}.${name}ID)) $dir"
        );
    }
    ...
    {
        $tableAlias = static::getTableAlias();
        return array("IF(StatusID IS NULL, 1, 0), (SELECT Title FROM asset_statuses Status WHERE Status.ID = ${tableAlias}.StatusID)" . $dir);
    }

    public static function sortLocation($dir)
    {
        $tableAlias = static::getTableAlias();
        return array("IF(LocationID IS NULL, 1, 0), (SELECT Title FROM locations Location WHERE Location.ID = {$tableAlias}.LocationID) " . $dir);
    }
