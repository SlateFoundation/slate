...
class Report extends \VersionedRecord
{
	...
	public function getStandardsGrades()
	{
		$where = array(
			...
			,'CourseSectionID' => $this->CourseSectionID
			,'StudentID' => $this->StudentID
			,'EXISTS(SELECT swp.ID FROM standards_worksheet_prompts swp WHERE swp.PromptID = Slate_Progress_Standards_PromptGrade.PromptID AND swp.WorksheetID = (SELECT swa.WorksheetID FROM standards_worksheet_assignments swa WHERE swa.TermID = Slate_Progress_Standards_PromptGrade.TermID AND swa.CourseSectionID = Slate_Progress_Standards_PromptGrade.CourseSectionID))'
		);
